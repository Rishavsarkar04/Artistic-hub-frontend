...
- Admin page widths: list pages (tables) are `max-w-[1400px] mx-auto`, wide enough to use the space without spreading columns apart; form and detail pages are `max-w-7xl mx-auto`. The shop side, including the account page, matches the navbar at `max-w-7xl`.
...
- The theme is **flame** (red and golden yellow), not black or brown: warm candle-light background, very dark red text, `ink`/`primary` = flame red for actions and selected states, `glow` = golden yellow for highlights, eyebrows and the focus ring (use `text-glow`/`bg-glow`, not hex), and `ember` = deep flame red for large dark surfaces (footer, admin sidebar, photo backdrops, dialog overlays). Don't use `bg-ink` for big areas; use `bg-ember` or `.bg-ember-glow` (red lit by a golden glow from below). Primary buttons (and the navbar cart) use `.bg-flame`, the red-to-orange gradient with a yellow glow; keep white text on it at 4.5:1, so don't push its end towards yellow. `destructive` is wine so delete never looks like a normal action. Avoid browns and greys. All of it lives in `src/index.css`.
...
- Typography is sized for readability: body text 17px (`1.0625rem`, line height 1.65, Geist at weight 450), and `text-xs`/`text-sm`/`text-base` are two steps up from Tailwind's defaults (14px / 16px / 17px, set in `@theme` in `index.css`). Nothing smaller than `text-[0.8125rem]` (13px). Write font sizes in `rem`, not `px`, so text follows the reader's browser font size. Muted text is `#6B2A0C` (about 10:1 on the background); text on the red surfaces uses light gold `#F5D9A8` or white at 75%+ opacity, never lower. Headings use `text-wrap: balance` and paragraphs `pretty`; don't cap short labels with a pixel `max-w` (they wrap at larger sizes).
